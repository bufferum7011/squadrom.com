<?php
    ...
    if(isset($_POST["enter_register"])) {
        try {
            ...
            exec_sql("INSERT INTO `Login` (`Email`, `Password`, `Token`, `Nickname`)
                        VALUES('$email_register', '$password_register', '$token_register', '$register_login')");
    ...
    if(isset($_POST["product_post"])) {
        ...
        try {
            $token = get_token_full();
            $data = date("d.m.Y");
            exec_sql("INSERT INTO `Showcase` (`Token`, `Title`, `Description`, `Price`, `Link_img`, `Time`)
            VALUES('$token', '{$_POST["product_title"]}', '{$_POST["product_desc"]}', '{$_POST["product_price"]}', '$json', '$data')");
        }
